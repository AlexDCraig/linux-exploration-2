...
void eat(char* name, int leftForkPos, int rightForkPos)
{
	...
	printf("Name: %s. Seat: %d. Status: EATING for %d seconds. Forks: HAS Left fork, position: %d. HAS Right fork, position: %d.\n", name, leftForkPos, random, leftForkPos, rightForkPos);
	usleep(randomTime);
}
...
void think(char* name, int pos)
{
	...
	printf("Name: %s. Seat: %d. Status: THINKING, %d seconds. Forks: HAS no forks.\n", name, pos, random);
	usleep(randomTime);
}

// After eating, put the forks down. Lock the left and right forks, adjust the semaphore, unlock the forks
void put_forks(char* philName, int philPosition, int rightForkPos)
{
	// Lock the forks
...
void get_forks(struct philosopher* phil, char* philName, int philPosition)
{
	...
	{
		printf("Name: %s. Seat: %d. Status: HUNGRY. Forks: HAS left fork, position %d.\n", philName, philPosition, philPosition);		
		...
		printf("Name: %s. Seat: %d. Status: HUNGRY. Forks: HAS left fork, position %d, checking on right fork, position: %d.\n", philName, philPosition, philPosition, rightForkPos);
		...
		if (isForkAvailable(rightForkPos) == true) 
		{
			...
			put_forks(philName, philPosition, rightForkPos);
		...
		{
			printf("Name: %s. Seat: %d. Status: HUNGRY. Forks: Has left fork, position %d. Can't have right fork, position %d. GIVING UP LEFT FORK.\n", philName, philPosition, philPosition, rightForkPos);
	...
	{
		printf("Name: %s. Seat: %d. Status: HUNGRY. Forks: Cannot have left fork, position %d, so won't check on his right fork, position %d.\n", philName, philPosition, philPosition, getRightForkPosition(philPosition));
		pthread_mutex_unlock(&forkLocks[philPosition]);
	}		 
...
void* lifeAsAPhilosopher(void* arg)
{
	...
	while(1)
	{
		...
		get_forks(phil, philName, philPosition);	
