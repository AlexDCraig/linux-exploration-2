...
static void sbd_transfer(struct sbd_device *dev, sector_t sector,
	...
	*/
	...
	{	
		...
		printk("WRITE REQUEST. Unencrypted form:\n");
		
		for (k = 0; k < nbytes; k++)
		{
			printk("%u", (unsigned) *(source + k));
		}

		printk("\n");

		// Print data after encryption
		printk("WRITE REQUEST: Encrypted form:\n");

		for (k = 0; k < nbytes; k++)
		{
			printk("%u", (unsigned) *(destination + k));
		}
	...
	else
	{
		...
		source = dev->data + offset; // Buffer that holds the ciphertext.
		...
		printk("READ REQUEST: Encrypted form:\n");
		...
		printk("READ REQUEST: Decrypted form:\n");
